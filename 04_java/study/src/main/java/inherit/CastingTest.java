...
public class CastingTest {
    public static void main(String[] args) {
        ...
        // 같은 패키지 내에 있는 것은 new 해서 사용 가능
        // 다른 패키지에 있으면 위치를 알려주어야 한다.
        List list = new ArrayList(); // 가변 길이, 다양한 타입을 넣을 수 있음

        // List list --> List 인터페이스에 넣은 대로 구현한 클래스는 다 집어넣을 수 있음.
        // 자동 형변환
        //ArrayList list2 = new LinkedList(); // 형제관계에서는 대입 불가능
        // public void add (object o) : object o 라고 하는 매개변수는 모든타입이 자동 형변환되어 들어감
        list.add("ABC"); // object는 클래스 다이어그램에서 최상위 위치
        // Object 타입: 변수에는 모든 타입의 데이터를 다 집어넣을 수 있다.
        list.add(new Random());
        list.add(new int[3]);
        list.add(new String("김국민")); //Object <-- String

        //형변환 -> 변수와 관련있는 단어!
        //자바에서는 다른 타입의 변수를 넣을 때 타입이 변환되어 들어감. (형변환)
        //list.add(100); // Object <-- Integer <- (오토박싱, 클래스로 바뀜)
        //변수 언제 쓰나??
        // 1) "클래스의 필드", 2) "매개변수"를 쓸때 형변환을 많이 씀
