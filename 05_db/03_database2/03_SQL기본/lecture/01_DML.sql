/*
 DML (Data Manipulation Language)
 -- 데이터 조작언어, 테이블에 값을 삽입, 수정, 삭제하는 SQL
 */

/*
Insert
- 새로운 행을 추가하는 구문
- null 허용 가능한 컬럼이나 auto increment가 있는 칼럼 제외하고
   insert하고 싶은 데이터 컬럼을 지정해서 Insert 가능

- Primary Key: 기본키, 유일성과 최소성을 만족, 해당 행을 식별할 때 사용되는 값
*/

insert into
    employees
values (1,20250514,'연아','최','F',20250514);

-- 컬럼을 명시하면 INSERT시 데이터의 순서 변경이 가능

insert into
    employees(emp_no, birth_date, first_name, last_name, gender, hire_date)
values (2,20250514,'민정','최','F',20250514);


-- 여러 컬럼을 한번에 insert
insert into
    employees(emp_no, birth_date, first_name, last_name, gender, hire_date)
values
    (3,20250514,'성호','박','F',20250514),
    (4,20250514,'정원','양','F',20250514);


-- update
-- 테이블에 기록된 컬럼의 값을 수정하는 구문

select
    *
from
    employees
where last_name = '성호';

update employees -- employees 업데이트할거다
set -- 이렇게 바꿀거다
    last_name = '박',
    first_name = '성호'
where
    last_name = '성호';

-- Delete: 테이블의 행을 삭제하는 구문

Delete
from employees
where last_name = '박'

-- Replace: insert 할때 primary key 혹은 unique 키가

value